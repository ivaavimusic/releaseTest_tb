...
async function searchGenesisAddress(symbol) {
  try {
    ...
    console.log(`🎯 Searching for G-prefixed: ${gPrefixedSymbol.toUpperCase()}`);
    console.log(`🎯 Searching for plain: ${plainSymbol.toUpperCase()}`);
    ...
    if (!match) {
      console.log(`❌ G-prefixed not found, trying plain symbol...`);
      match = genesisData.find(item => item.virtual.symbol === plainSymbol.toUpperCase());
    }

    if (match) {
      const foundSymbol = match.virtual.symbol;
      console.log(`✅ Genesis Address for ${foundSymbol}: ${match.genesisAddress}`);
      return match.genesisAddress;
    } else {
      console.log(`❌ No genesis found for either G-${gPrefixedSymbol} or ${plainSymbol}`);
      return null;
    }
  } catch (error) {
  ...
  async initializePhase1(args) {
    ...
    if (this.config.INPUT_TYPE === 'GENESIS_TICKER') {
      ...
      try {
        ...
        if (!genesisAddress) {
          console.log(`⚠️ No Genesis contract found for ticker: ${this.config.GENESIS_TICKER_SYMBOL}`);
          console.log(`🔄 Falling back to regular ticker resolution...`);
          ...
          try {
            ...
            if (tokenCA) {
              console.log(`✅ Fallback successful: ${this.config.GENESIS_TICKER_SYMBOL} → ${tokenCA}`);
              ...
              console.error(`❌ Fallback failed: No regular ticker found for ${this.config.GENESIS_TICKER_SYMBOL}`);
              return false;
            }
          } catch (fallbackError) {
          ...
          console.log(`✅ Genesis contract found for ${this.config.GENESIS_TICKER_SYMBOL}: ${genesisAddress}`);
          console.log(`🎯 JeetBot will monitor Genesis contract: ${genesisAddress}`);
          console.log(`⏳ Waiting for token deployment from this Genesis contract...`);
        ...
        console.log(`🔄 Attempting fallback to regular ticker resolution...`);
        ...
        try {
          ...
          const tokenCA = await TokenInfoResolver.resolveTokenCA('TICKER', this.config.GENESIS_TICKER_SYMBOL);
          if (tokenCA) {
            console.log(`✅ Fallback successful after error: ${this.config.GENESIS_TICKER_SYMBOL} → ${tokenCA}`);
            // Convert to TOKEN_CA input type and continue
            this.config.INPUT_TYPE = 'TOKEN_CA';
            this.config.input = tokenCA;
            this.config.TOKEN_CONTRACT_ADDRESS = tokenCA;
            ...
            console.error(`❌ Fallback failed after error: No regular ticker found for ${this.config.GENESIS_TICKER_SYMBOL}`);
            return false;
          }
        } catch (fallbackError) {
          console.error(`❌ Fallback error after genesis error: ${fallbackError.message}`);
          return false;
        }
      }
    }
